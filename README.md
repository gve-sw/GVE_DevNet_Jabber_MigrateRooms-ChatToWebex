...
## Installation/Configuration
 ...
 3. Make sure you have [Python](https://www.python.org/downloads/) installed. This app was written with Python 3.8, and the libraries in the requirements.txt file are compatible with Python 3.8. To ensure a smooth experience, download a version of Python 3.8.
 ...
 5. Configure the configuration variables in ```config.py``` file:
    ...
    3. Set up the boolean variable *CREATE_WEBEX_ROOMS* to choose to migrate the detected Jabber rooms to Webex, or just test the script's ability to read Jabber's chat data.
        ...
        - Also if set to True, and in case *Jabber's Chat IM address* domain used is different than the user's email domain used in *Webex*, set up the following two variables accordingly:
          ...
          WEBEX_DOMAIN = ""
          ```
    4. Set up the boolean variable *INCLUDE_JABBER_WEBEX_MAP* to choose if you have an Excel file that maps the Jabber IDs of the users to their Webex IDs.
        ```python
        INCLUDE_JABBER_WEBEX_MAP = True
        ```
        - If set to True, you will be expected to provide an Excel file named `jabber_to_webex.xlsx` in this directory that contains the Jabber IDs and Webex IDs of the users in the following format.

        ![/IMAGES/jabber_to_webex_sheet.png](/IMAGES/jabber_to_webex_sheet.png)

        - A template of this Excel file with the name `jabber_to_webex.xlsx` is provided in this repository for you to fill out as needed.

    5. Set up the following two varibales to have the paths to two local folders to store:
